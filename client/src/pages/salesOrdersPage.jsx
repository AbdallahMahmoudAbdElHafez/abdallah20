...
export default function SalesOrdersPage() {
    ...
    const handleEdit = useCallback(async (order) => {
        try {
            ...
            const res = await dispatch(fetchSalesOrderItems({ sales_order_id: order.id })).unwrap();
    ...
    const handleUpdate = useCallback(async (payload) => {
        try {
            ...
            console.error('فشل في تحديث أمر المبيعات:', error);
        } finally {
            setActionLoading(false);
        }
    ...
    const handleAdd = useCallback(async (payload) => {
        try {
            ...
            console.error('فشل في إضافة أمر المبيعات:', error);
        } finally {
            setActionLoading(false);
        }
    }, [dispatch]);
    ...
    const handleViewInvoice = useCallback((orderId) => {
        navigate(`/sales-invoices?sales_order_id=${orderId}`);
    }, [navigate]);
        ...
        return (
            <Box p={2}>
                <Alert severity="error" sx={{ mb: 2 }}>
                    فشل في تحميل أوامر المبيعات: {error}
                </Alert>
                <Button
                    variant="contained"
